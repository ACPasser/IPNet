...
class DataLoader:
    ...
    def _split_train_val_test(self) -> None:
        ...
        if not (0 < self.train_ratio < 1 and 0 < self.val_ratio < 1):
            raise ValueError(f"训练/验证比例必须在(0,1)范围内! train={self.train_ratio}, val={self.val_ratio}")
        if self.train_ratio + self.val_ratio >= 1:
            raise ValueError(f"训练+验证比例需小于1! 当前总和={self.train_ratio + self.val_ratio}")
        ...
        # val graph
        val_edges_filtered = [
            (u, v, attr) for u, v, attr in val_edges_raw
        ...
        # test graph
        test_edges_filtered = [
            (u, v, attr) for u, v, attr in test_edges_raw
            if u in train_nodes and v in train_nodes
        ]
        ...
        self._print_split_stats(
            total_edges,
            len(train_edges_raw), len(val_edges_raw), len(test_edges_raw),
            len(self.train_graph.edges()), len(self.val_graph.edges()), len(self.test_graph.edges())
        )
    ...
    def _print_split_stats(
        self,
        total_edges: int,
        train_raw: int, val_raw: int, test_raw: int,
        train_filtered: int, val_filtered: int, test_filtered: int
    ) -> None:
        """打印数据集分割统计信息（格式化输出）"""
        ...
        # 格式化输出
        print("\n📊 数据集划分完成")
        print(f"   ├─ 原始数据 | 总边数: {total_edges:>6} | 训练集: {train_raw:>6} ({train_ratio_raw:.2%}) | 验证集: {val_raw:>6} ({val_ratio_raw:.2%}) | 测试集: {test_raw:>6} ({test_ratio_raw:.2%})")
        print(f"   ├─ 过滤后   | 总边数: {total_filtered:>6} | 训练集: {train_filtered:>6} ({train_ratio_filtered:.2%}) | 验证集: {val_filtered:>6} ({val_ratio_filtered:.2%}) | 测试集: {test_filtered:>6} ({test_ratio_filtered:.2%})")
        print(f"   └─ 过滤信息 | 共过滤掉 {filtered_out} 条边（验证/测试集含训练集外节点）")
    ...
    def preprocess(
        ...
        if not (0 < mask_ratio < 1) and task_type == "I":
            raise ValueError(f"归纳式任务mask_ratio必须在(0,1)之间! 当前值: {mask_ratio}")
        
        if task_type == "I":
            # 节点Mask + 过滤测试/验证集
            self.train_graph_masked, self.val_graph_filtered, self.test_graph_filtered = self.mask_nodes_and_filter(
    ...
    def _preprocess_single_graph(
        ...
        print(f"\n📊 {name}集样本统计（边去重后）:")
        print(f"   - 正样本数: {pos_num} (去重前: {raw_pos_num}, 重复率: {dup_rate:.2%})")
        print(f"   - 负样本数: {neg_num} (平衡采样)")
        print(f"   - 总样本数: {total_num} (正负样本比: 1:1)")
        print(f"   - 节点数: {len(nodes)} | 邻接表大小: {len(adj)}")
    ...
    def mask_nodes_and_filter(
        ...
        val_edges_to_keep = [
            (u, v, k) for u, v, k in val_graph.edges(keys=True)
        ...
        # 3. 过滤测试集：逻辑同验证集
        test_edges_to_keep = [
            (u, v, k) for u, v, k in test_graph.edges(keys=True)
            if u in sampled_nodes_set or v in sampled_nodes_set
        ]
        ...
        print(f"\n✅ [Inductive Masking] 从训练集中Mask掉{len(sampled_nodes)}个节点 (占比: {mask_ratio_actual:.1f}%)")
        print(f"  ├─ 训练图 (Train): 节点 {orig_train_nodes} → {new_train_nodes} | 边 {orig_train_edges} → {new_train_edges}")
        print(f"  ├─ 验证图 (Val)  : 边 {orig_val_edges} → {new_val_edges} (仅保留Mask节点相关边)")
        print(f"  └─ 测试图 (Test) : 边 {orig_test_edges} → {new_test_edges} (仅保留Mask节点相关边)")

        if new_val_edges == 0:
            print("  ⚠️ 警告: 过滤后验证集边数为 0! 建议降低mask_ratio或检查数据集连通性。")
        if new_test_edges == 0:
            print("  ⚠️ 警告: 过滤后测试集边数为 0! 建议降低mask_ratio或检查数据集连通性。")
    ...
    def extract_interaction_seqs(
        ...
        if graph is not None:
            # 外部传入的graph优先
            if not isinstance(graph, nx.MultiGraph):
                raise TypeError(f"传入的graph必须是nx.MultiGraph类型, 当前为{type(graph)}")
            target_graph = graph
            desc = "自定义图数据"
        else:
            if task_type == "T":
        ...
        for node in target_graph.nodes():
            ...
            for neighbor, edge_dict in target_graph[node].items():
                for edge_attr in edge_dict.values():
                    if self.time_attr not in edge_attr:
                        raise KeyError(f"边({node},{neighbor})缺少'{self.time_attr}'属性")
            ...
            unique_neighbors_with_time.sort(key=lambda x: x[1], reverse=True) # 保证时间倒序

            node_interaction_seq[node] = unique_neighbors_with_time    

        # 统计信息
        total_nodes = len(node_interaction_seq)
        avg_seq_len = sum(len(seq) for seq in node_interaction_seq.values()) / total_nodes if total_nodes > 0 else 0
        print(f"\n✅ 交互序列提取完成")
        print(f"   ├─ 数据源: {desc}")
        print(f"   ├─ 处理节点数: {total_nodes}")
    ...
    def extract_ctx_window(
        ...
        if walk_len <= 0 or walk_num <= 0:
            raise ValueError(f"游走长度/次数必须大于0! walk_length={walk_len}, num_walks={walk_num}")

        if graph is not None:
            # 外部传入的graph优先
            if not isinstance(graph, nx.MultiGraph):
                raise TypeError(f"传入的graph必须是nx.MultiGraph类型, 当前为{type(target_graph)}")
            target_graph = graph
        else:
            if task_type == "T":
                # 直推式：使用训练+验证集合并图
        ...
        context_chunks = Parallel(n_jobs=workers)(
            delayed(self._parallel_generate_context)(
                ...
                use_linear=True,
                half_life=1.0
            ) for idx, chunk in enumerate(num_walks_chunks)
        )
        ...
        print(f"✅ 上下文窗口提取完成")
        print(f"   ├─ 游走配置: 次数={walk_num} | 长度={walk_len} | CPU进程数={workers}")
        total_contexts = sum(len(v) for v in context_dict.values())
        avg_seq_per_node = total_contexts / len(context_dict) if len(context_dict) > 0 else 0
        print(f"   ├─ 提取结果: 覆盖节点数={len(context_dict)} | 总序列数={total_contexts}")
        print(f"   └─ 统计信息: 单节点平均序列数={avg_seq_per_node:.2f}")
        return context_dict
    ...
    def _precompute_transition_probs(
        ...
        nodes_generator = graph.nodes() if quiet else tqdm(
            graph.nodes(), 
            desc=f"\n🔄 计算游走转移概率", 
            ncols=100,
            leave=True
        ...
        for source in nodes_generator:
            ...
            for current_node in graph.neighbors(source):
                ...
                for dest in graph.neighbors(current_node):
                    edge_weight = graph[current_node][dest][0].get(const_keys["weight"], 1.0)
                    if dest == source:  # Backwards probability
                        ss_weight = edge_weight * node_act.get(dest, 1.0) / p
                    elif dest in graph[source]:  # If the neighbor is connected to the source
                ...
                d_graph[current_node][const_keys["neighbors_time"]] = self._get_neighbor_time_map(
                ...
                while len(walk) < walk_length:
                    ...
                    try:
                        if len(walk) == 1:
                            ...
                            probs = d_graph[walk[-1][0]][const_keys["probabilities"]][walk[-2][0]]
                    except KeyError as e:
                        raise KeyError(f"节点{walk[-1][0]}的转移概率不存在！{e}")
                    
                    walk_options = self._get_valid_walk_options(
                        d_graph, walk[-1][0], probs, const_keys, last_time, filter_func=lambda t: t > last_time
                    )
                    # Skip dead end nodes
                    if not walk_options:
                        break
                    ...
                    final_probs = self._compute_final_probabilities(walk_options, use_linear, half_life, last_time)
        ...
        for _ in range(num_walks):
            ...
            for source in nodes:
                ...
                while len(walk) < half_len:
                    try:
                        probs = d_graph[walk[-1][0]][const_keys["first_travel"]] if len(walk) == 1 else \
                                d_graph[walk[-1][0]][const_keys["probabilities"]][walk[-2][0]]
                    except KeyError as e:
                        raise KeyError(f"节点{walk[-1][0]}的转移概率不存在！{e}")
                ...
                while len(walk) < walk_length:
                    try:
                        probs = d_graph[walk[0][0]][const_keys["first_travel"]] if len(walk) == 1 else \
                                d_graph[walk[0][0]][const_keys["probabilities"]][walk[1][0]]
                    except KeyError as e:
                        raise KeyError(f"节点{walk[0][0]}的转移概率不存在！{e}")
                    
                    walk_options = self._get_valid_walk_options(
                        d_graph, walk[0][0], probs, const_keys, last_time, filter_func=lambda t: t < last_time
                    )
    ...
    def estimate_walk_params(
        ...
        if not quiet:
            density_desc = "极稀疏" if density < DENSITY_THRESHOLD_1 else \
                          "中等稀疏" if density < DENSITY_THRESHOLD_2 else "稠密"
            print(f"\n📊 游走参数自动估算完成（基于网络特征）")
            print(f"   ├─ 网络基础特征:")
            print(f"   │  - 节点数: {num_nodes:,} | 总边数(含重复): {total_edges:,} | 唯一边数: {stats['num_unique_edges']:,}")
            print(f"   │  - 平均度: {avg_degree:.1f} | 平均交互频率: {avg_interact_freq:.1f} | 平均路径长度: {avg_path_len:.1f}")
            print(f"   │  - 网络密度: {density:.4f} ({density_desc})")
            print(f"   └─ 推荐游走参数:")
            print(f"      - WALK_NUM (每节点游走次数): {walk_num}")
            print(f"      - WALK_LEN (单次游走长度): {walk_len}")
    ...
    def _calculate_graph_stats(
        ...
        avg_interact_freq = total_edges / num_unique_edges if num_unique_edges > 0 else 1.0
