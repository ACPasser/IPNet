...
def train(
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.8, patience=1)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx in tqdm(range(num_batches), desc=f"Epoch {epoch + 1}/{epochs}"):
            ...
            train_neg_edges = np.array(negative_sampling(train_nodes, train_adj, curr_batch_size, seed=seed))
            ...
            with torch.no_grad():
                ...
                true_label = np.concatenate([np.ones(curr_batch_size), np.zeros(curr_batch_size)])
                pred_label = pred_score > 0.5

                # 更新指标
                epoch_metrics["acc"].append((pred_label == true_label).mean())
                epoch_metrics["ap"].append(average_precision_score(true_label, pred_score))
        ...
        val_edges_neg = np.array(negative_sampling(
            val_nodes, val_adj, val_data["neg_num"], seed=seed
        ))
        np.random.shuffle(val_edges_neg)
        val_acc, val_ap, val_f1, val_auc = eval_one_epoch(model, val_edges_pos, val_edges_neg, batch_size)
        ...
        if epoch < 5 or (epoch + 1) % 5 == 0:  # 前5轮必打，之后每5轮打一次
            with torch.no_grad():
                ...
                val_sample_all = np.concatenate([val_sample_pos, val_sample_neg], axis=0)
                ...
                # print(f"Val正样本原始logits（前10个）: {np.round(pos_logits, 4)} | 均值: {np.mean(pos_logits):.4f}")
                # pos_pred = val_sample_scores[:10]
                # print(f"Val正样本预测概率（前10个）: {np.round(pos_pred, 4)} | 均值: {np.mean(pos_pred):.4f}")

                # # 负样本预测
                # neg_pred = val_sample_scores[-10:]
                # neg_pred = val_sample_scores[-10:]
                # print(f"Val负样本节点对（前10个）: {val_sample_neg.tolist()}")
                # print(f"Val负样本预测概率（前10个）: {np.round(neg_pred, 4)} | 均值: {np.mean(neg_pred):.4f}")
                # # 正负样本概率重叠度（>0表示区分度好，<0表示有混淆）
                # overlap = np.min(pos_pred) - np.max(neg_pred)
                # print(f"Val正负样本概率重叠度（正最小-负最大）: {overlap:.4f} (＞0=区分度好)")

        print(f"\n=== Epoch {epoch+1} Results (lr: {current_lr:.6f}) ===")
        print(f"Train Loss: {np.mean(epoch_metrics['loss']):.4f}")
        print(f"Train Acc: {np.mean(epoch_metrics['acc']):.4f} | Val Acc: {val_acc:.4f}")
        print(f"Train AUC: {np.mean(epoch_metrics['auc']):.4f} | Val AUC: {val_auc:.4f}")
...
def eval_one_epoch(
    ...
    all_test_labels = np.concatenate([np.ones(len(pos_edges)), np.zeros(len(neg_edges))])
...
def get_device(gpu_flag: int) -> torch.device:
    ...
    if gpu_flag == -2:
        ...
        print(f"✅ Forced to use device: CPU")
    
    elif gpu_flag == -1:
        if torch.backends.mps.is_available() and torch.backends.mps.is_built():
            device = torch.device("mps")
            print(f"📌 Using device: Apple MPS (Apple Silicon acceleration)")
        else:
            device = torch.device("cpu")
            print(f"⚠️ MPS not available (need PyTorch ≥2.0 + Apple Silicon), fallback to CPU")
        ...
        if torch.cuda.is_available():
            ...
            device = torch.device("cpu")
            print(f"⚠️ NVIDIA GPU not available, fallback to CPU")
...
def main(args: argparse.Namespace) -> None:
    """
    主函数：参数解析、数据加载、模型训练、结果保存
    """
    task_type_name = "Transductive (直推式)" if args.TASK_TYPE == "T" else "Inductive (归纳式)"
    ...
    paths = {
        ...
        # 通过 data/{DATASET}/{DATASET}.py 生成
        "graph": f"data/{args.DATASET}/0.origin/graph.csv",
        "nodes": f"data/{args.DATASET}/1.nodes_set/nodes.csv"
    }

    # 模型保存路径
    timestamp = datetime.fromtimestamp(time.time()).strftime("%Y-%m-%d_%H_%M_%S")
    ckpt_dir = os.path.join(paths["model_dir"], "saved_checkpoints", timestamp) # 每个检查点
    best_model_dir = os.path.join(paths["model_dir"], "best_models", timestamp) # 验证集上的最佳模型
    best_model_path = os.path.join(best_model_dir, "best-model.pth")
    check_and_make_path(ckpt_dir)
    ...
    graph = get_nx_graph(paths["graph"], paths["nodes"], sep="\t")
    ...
    if need_estimate:
        recommend_walk_num, recommend_walk_len = data_loader.estimate_walk_params(task_type=args.TASK_TYPE)
        param_updates = []
        if args.WALK_NUM == -1:
        ...
        print(f"📏 模型使用指定的游走参数：游走次数: {final_walk_num}, 游走长度: {final_walk_len}")
    ...
    if args.VERSION == "w2v":
        ...
        print("\n✅ Word2Vec训练完成")
        print(f"   ├─ 节点数: {len(word2vec.wv.index_to_key)} | 特征维度: {word2vec.wv.vector_size}")
    ...
    # 6. 训练
    # 训练/验证/测试数据预处理
    train_data, val_data, test_data = data_loader.preprocess(args.TASK_TYPE, args.MASK_RATIO, args.SEED)
    print("\n🚀 Start training...")
    train(
        model=model,
    ...
    # 7. 测试
    print("\n📝 Final testing...")
    test_neg = np.array(negative_sampling(
        test_data["nodes"], 
        test_data["adj"], 
        test_data["neg_num"], 
        hard_ratio=0.0, 
    ...
    test_acc, test_ap, test_f1, test_auc = eval_one_epoch(model, test_data["edges"], test_neg, args.BATCH_SIZE)
    ...
    if args.TASK_TYPE == "I":
        mask_percent = int(args.MASK_RATIO * 100) if args.MASK_RATIO * 100 == int(args.MASK_RATIO * 100) else args.MASK_RATIO * 100
        mask_ratio_str = f"{mask_percent:.0f}%" if args.MASK_RATIO * 100 % 1 == 0 else f"{mask_percent:.1f}%"
        core_param_desc_list.append(f"MR-{mask_ratio_str}")
    result_dir = os.path.join(paths["result_dir"], "_".join(core_param_desc_list))
    check_and_make_path(result_dir)
    ...
    with open(result_path, "a+", newline="") as f:
        writer = csv.writer(f)
        if need_header:
            writer.writerow(["Task_Type", "Acc", "AUC", "AP", "F1", "Time(s)", "Seed", "Best_Model_Path"])
        writer.writerow([
            args.TASK_TYPE,
            test_acc * 100,
            test_auc * 100,
...
if __name__ == "__main__":
    ...
    # 基础配置
    parser.add_argument("--seed", dest="SEED", type=int, default=19, help="Random seed for reproducibility (default: 19)")
    ...
    if args.PADDING_NODE < 0:
        raise ValueError(f"Padding node ID must be non-negative! Current: {args.PADDING_NODE}")

    if args.TASK_TYPE == "I":
        if not (0 < args.MASK_RATIO < 1):
            raise ValueError(f"Inductive task requires mask_ratio in (0,1)! Current: {args.MASK_RATIO}")
